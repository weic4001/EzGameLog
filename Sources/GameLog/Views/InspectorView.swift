...
struct InspectorView: View {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            HStack(spacing: 6) {
                                ...
                                if issue.occurrenceCount > 1 {
                                    Text("重复 \(issue.occurrenceCount) 次")
                                }
    ...
    private var recoverableRecordingSection: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("待恢复录屏", systemImage: "externaldrive.badge.exclamationmark")
                .font(.headline)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let event = model.selectedEvent {
                if model.selectedEventIDs.count > 1 {
                    Label("已选择 \(model.selectedEventIDs.count) 条；下方显示第一条", systemImage: "checklist")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                HStack {
                    ...
                    Button {
                        ...
                        Label(
                            model.bookmarkedEventIDs.contains(event.id) ? "取消书签" : "添加书签",
                ...
                if let issue = model.selectedDiagnosticIssue {
                    Label(
                        "\(issue.kind.title) · \(issue.occurrenceCount) 次",
                ...
                if let receivedAt = event.receivedAtHostTime {
                    LabeledContent("主机接收", value: receivedAt.formatted(
    ...
    private var evidenceSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if model.evidence.isEmpty {
                Text("开始会话后使用工具栏的截图或录屏按钮，证据会保存到当前会话目录。")
                    .foregroundStyle(.secondary)
            } else {
...
private struct IncidentEditorCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 9) {
            Label("问题标记", systemImage: "exclamationmark.bubble.fill")
                .font(.headline)
                .foregroundStyle(.orange)
            TextField("问题名称", text: $title)
            TextField("复现步骤或补充说明", text: $note, axis: .vertical)
                .lineLimit(2...5)
            HStack {
                if let offset = incident.recordingOffset {
                    Label(
                        "录屏 \(offset.formatted(.number.precision(.fractionLength(1)))) 秒",
...
private struct EvidenceCard: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack {
                ...
                if let duration = item.duration {
                    Text(duration.formatted(.number.precision(.fractionLength(1))) + " 秒")
                }
    ...
    private func secondaryEvidenceActions(compact: Bool) -> some View {
        HStack(spacing: compact ? 10 : 12) {
            Button(compact ? "导出" : "导出…") {
                Task { await model.exportEvidence(item) }
            }
            .help("导出这个证据文件")

            Button(compact ? "Finder" : "在 Finder 中显示") {
                NSWorkspace.shared.activateFileViewerSelecting([item.fileURL])
            }
            .help("在 Finder 中显示这个文件")
        }
        .buttonStyle(.link)
        .lineLimit(1)
        .fixedSize(horizontal: true, vertical: false)
