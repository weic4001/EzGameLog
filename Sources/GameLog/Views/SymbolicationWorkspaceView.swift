...
struct SymbolicationWorkspaceView: View {
    ...
    var body: some View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                HStack {
                    Text(String(localized: "NDK Native 符号化"))
                        .font(.title2.weight(.semibold))
                    Spacer()
                    Picker(String(localized: "目标包"), selection: $model.symbolPackage) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(
                        model.resolvedSymbolizerPath.isEmpty
                            ? String(localized: "尚未找到 Android NDK llvm-symbolizer")
                            : model.resolvedSymbolizerPath
                    )
                    .font(.caption.monospaced())
                    .textSelection(.enabled)
                    Text(String(localized: "优先自动读取 ANDROID_NDK_HOME / ANDROID_HOME，也可手动选择。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                Button(String(localized: "选择工具…")) {
    ...
    private var symbolRoots: some View {
        GroupBox(String(localized: "符号目录")) {
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    Text(String(localized: "\(model.visibleSymbolFileCount) 个 ELF 库已建立索引"))
                        .foregroundStyle(.secondary)
                    Spacer()
                ...
                if model.visibleSymbolRoots.isEmpty {
                    Text(String(localized: "尚未为该目标添加符号目录。选择包含未剥离 .so 的目录即可递归索引 ABI 和 GNU Build ID。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ForEach(model.visibleSymbolRoots) { root in
                    HStack {
                        ...
                        Text(root.path)
                            ...
                            .textSelection(.enabled)
                        Spacer()
                        Button(String(localized: "移除索引"), role: .destructive) {
                            Task { await model.removeSymbolRoot(root.id) }
                        }
                        .help(String(localized: "只移除 GameLog 索引，不删除源符号文件"))
                    }
                }
            }
            .padding(4)
    ...
    private var sessionSymbolication: some View {
        GroupBox(String(localized: "会话符号化")) {
            VStack(alignment: .leading, spacing: 10) {
                HStack {
                    sessionPicker
                    ...
                    Button(String(localized: "符号化所选会话")) {
                ...
                if let report = model.selectedSymbolicationReport {
                    ...
                    HStack {
                        Picker(String(localized: "结果"), selection: $resultFilter) {
                        ...
                        Spacer()
                        Button(String(localized: "复制完整结果")) {
                            model.copySelectedSymbolicationReport()
                        }
                        Button(String(localized: "导出文本…")) {
                    ...
                    if visibleFrames.isEmpty {
                        Text(String(localized: "当前筛选下没有 Native 帧。"))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                            .padding(.vertical, 8)
                    } else {
                    ...
                    Text(String(localized: "选择包含 Native Crash 的会话并执行符号化，结果会保存到会话目录的 symbolication.json。"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func symbolicationIssues(
        ...
        if missing + unresolved + failed > 0 {
            GroupBox(String(localized: "未解析原因与处理建议")) {
                VStack(alignment: .leading, spacing: 7) {
                    if !report.missingLibraryNames.isEmpty {
                        Label(
                            String(localized: "缺少或无法唯一匹配：")
                                + report.missingLibraryNames.joined(separator: "、"),
                            systemImage: "questionmark.folder"
                        )
                        Text(String(localized: "添加包含对应 ABI 未剥离 .so 的目录；同名库较多时需保留 Build ID。"))
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    if unresolved > 0 {
                        Label(
                            String(localized: "\(unresolved) 帧被 llvm-symbolizer 接收，但没有返回函数或源码位置。"),
                            systemImage: "questionmark.circle"
                        )
                    }
                    if failed > 0 {
                        Label(
                            String(localized: "\(failed) 帧执行失败；在下方帧详情查看超时、权限或工具错误。"),
    ...
    private var sessionPicker: some View {
        @Bindable var model = model
        return Picker(String(localized: "会话"), selection: $model.selectedSessionID) {
        ...
        HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                if let path = value.symbolFilePath {
                    Text(String(localized: "符号文件：\(path)"))
...
private enum SymbolicationResultFilter: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .symbolicated: String(localized: "已解析")
        case .alreadySymbolicated: String(localized: "日志已有")
        case .missingSymbolFile: String(localized: "缺少符号")
